...
def create_sqldb() -> None:
    ...
    try:
        ...
        logger.info("SQL database exists")
...
def cognito_user_jwt(email, password) -> None:
    ...
    cognito_client = utils.get_cognito_client()
    jwt = utils.cognito_user_jwt(cognito_client, email, password)
    log.info(f"\n\nCognito JWT\n\n{jwt}\n\n")


@click.command()
@click.option("--email", "-e", default="gen3@wisecode.ai", type=str)
@click.option("--password", "-p", default="Bitcoinrising2021!", required=True, type=str)
def wisecode_platform_login(email, password) -> None:
    """
    ...
    response = requests.post(
            f"{utils.get_env_var('FENCE_BASE_URL')}login/wisecode", headers={
...
def main() -> None:
    ...
    for command in [wisecode_platform_login, create_sqldb, set_up_sqldb, drop_sqldb, truncate_sqldb, create_cognito_user, cognito_user_jwt]:
